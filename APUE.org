...
*** TODO 12.4 Synchronization Attributes [1/4]
**** DONE 12.4.1 Mutex Attributes
     ...
     #+begin_quote
     use of the mutex can result in undeﬁned behavior, and applications waiting for it to be unlocked are effectively "stalled".
     #+end_quote
     ...
     * failure
     ...
     #+begin_quote
     If a recursive mutex is locked multiple times and used in a call to pthread_cond_wait, the condition can never be satisﬁed, because the unlock done by pthread_cond_wait doesn’t release the mutex.

...
**** DONE 11.6.6 Condition Variables
     ...
     #+begin_quote
     The condition itself is protected by a mutex. A thread must ﬁrst lock the mutex to change the condition state. Other threads will not notice the change until they acquire the mutex, because the mutex must be locked to be able to evaluate the condition.

...
** TODO Chapter 15. Interprocess Communication [7/12]
...
**** DONE 15.6.1 Identifiers and Keys
     ...
     *Whenever an IPC structure is being created, a key (<sys/types.h>::key_st) must be specified.*
     ...
     3. The client and the server can agree on a pathname and project ID (the project ID is a character value between 0 and 255) and call the function ~ftok~ to convert theses two values into a key.
...
**** DONE 15.6.4 Advantages and Disadvantages
     CLOSED: [2020-01-09 Thu 18:20]
     * message queue: data left until spcifically read or deleted by some process calling ~msgrcv~ or ~msgctl~, by someone executing the ~ipcrm~ command, or by the system being rebooted.
...
*** DONE 15.7 Message Queues
    ...
    #+begin_src c
      ...
      /// * IPC_SET copy the following fields from the structure pointed to by buf to the msqid_ds
      ...
      /// * IPC_NOWAIT can be specified, this is similar to the nonblocking I/O flag for file I/O.
      ///
      /// @retval 0 OK
      /// @retval -1 error
      ...
      /// * type < 0  the fire message on the queue whose message type is the lowest values
